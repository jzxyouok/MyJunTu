...
@interface HolidayHotelViewController ()<UITableViewDataSource,UITableViewDelegate>
{
    ...
    UILabel *_getInDataLabel;//入住日期
    UILabel *_getInWeedDayLabel;
    UILabel *_stayDayLabel;//居住时长
    UILabel *_getOutDataLabel;//离店日期
    UILabel *_getOutWeedDayLabel;
    UITextField *_keyTextF;//关键字文本
    UILabel *_priceLevelLabel;//价格星级
    UIView *_selectView;//星级／价格选择视图
    UIView *_priceSelectedView;//可移动的选择滑块
    UIView *_levelSelectedView;
    UILabel *_priceSelectedLabel;//可移动的选择滑块上的label
    UILabel *_levelSelectedLabel;
    UILabel *_priceItemLabel;//价格、星级选项
    UILabel *_levelItemLabel;
    NSString *_cityId;//城市id
    NSString *_level;//酒店星级
    NSString *_minPrice;//酒店最低价
    NSString *_maxPrice;//酒店最高价
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [_keyTextF resignFirstResponder];
    if (indexPath.row == 0) {
    ...
    }else if (indexPath.row == 2){//关键字
        [_keyTextF becomeFirstResponder];
    }
    else if (indexPath.row == 3){//星级价格选择
